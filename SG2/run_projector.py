...
def project_image(proj, targets, png_prefix, num_snapshots):
    ...
    while proj.get_cur_step() < proj.num_steps:
        print('\r%d / %d ... ' % (proj.get_cur_step()+1, proj.num_steps), end='', flush=True)
        latents = proj.step()
        if proj.get_cur_step() in snapshot_steps:
            misc.save_image_grid(proj.get_images(), png_prefix + 'step%04d.png' % proj.get_cur_step(), drange=[-1,1])
        if proj.get_cur_step() == proj.num_steps-1:
            latents = np.squeeze(latents, axis=0)
            with open("latents.txt", "ab") as f:
                np.savetxt(f, np.expand_dims(np.mean(latents, axis=0),axis=0), delimiter=',', fmt='%1.5f')
                f.write(b"\n")
    latents = np.squeeze(latents, axis=0)
    return latents
...
def project_real_images(network_pkl, dataset_name, data_dir, num_images, num_snapshots):
    ...
    for image_idx in range(num_images):
        ...
        all_latents.append(np.mean(project_image(proj, targets=images, png_prefix=dnnlib.make_run_dir_path('image%04d-' % image_idx), num_snapshots=num_snapshots),axis=0))
    for j in range(len(all_latents)-1):
        for i in range(j+1,len(all_latents)):
            ...
            print(f"Dot product between {j} and {i}: {np.dot(all_latents[j],all_latents[i])}")
            print(f"Cosine product between {j} and {i}: {spatial.distance.cosine(all_latents[j],all_latents[i])}")
