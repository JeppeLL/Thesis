...
def generate_images(network_pkl, seeds, truncation_psi, latents):
    ...
    if latents == None:
        ...
        for lat_idx, lat in enumerate(latents):
            ...
            tflib.set_vars({var: rnd.randn(*var.shape.as_list()) for var in noise_vars}) # [height, width]
            images = Gs.run(z, None, **Gs_kwargs) # [minibatch, height, width, channel]
            PIL.Image.fromarray(images[0], 'RGB').save(dnnlib.make_run_dir_path('lat%04d.png' % lat_idx))
#----------------------------------------------------------------------------
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_generate_images.add_argument('--latents', help='Txt file that contains list of latent vectors to produce pictures from [n x 512]. dim1 must be separated by "," and dim0 by " ".', default=None)
